...
df['answer'] = df['answer'].map(transform)  #apply the function to the column
...
ans = df['answer']              # create answer column manually
...
ans_all = ans.values        # get the answer
...
final_ans = ans_all[:9]    # get the answer in the csv
...
Briefly reflect on how this all went...
  + what value of k did you decide on for your kNN?
    I use a loop to find the best K, the program will decide the best one for me.
  + how smoothly did the kNN workflow go...
    It went pretty well.
  + thoughts on this "machine learning pipeline" ?
    It's useful because we can use the existing data to train it and then use it for predict the data we want to know.
...
Also, include the predicted labels of the 9 unlabeled irises!
  ...
  The prediction went well, only the first one is incorrect.
