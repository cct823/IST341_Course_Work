...
def scale (L, scale_factor):
    ...
    LC = [L[i]*scale_factor for i in range(len(L))]
...
def add_scale_2(L,M,L_scale,M_scale):
    ...
    LC = [ L[i]*L_scale + M[i]*M_scale for i in range(min (len(L),len(M)))]
    return LC

def add_scale_3(L,M,P,L_scale,M_scale,P_scale):

    LC = [L[i] * L_scale + M[i] * M_scale + P[i]*P_scale for i in range(min(len(L), len(M),len(P)))]
    return LC
...
# Reverse the sound by print out the list from right to left.
...
def volume(filename,scale_factor):    # not complete
    print("Playing the original sound...")
    play(filename)
    ...
    # this gets the midpoint and calls it x
    # x = len(samps) // 2
    newsamps = scale(samps,scale_factor)  # what's the first input for L?#
    newsr = sr
    new_sound_data = [newsamps, newsr]
...
def echo(filename, time_delay):
    ...
    Delay = [0] * int(sr*time_delay)
    samps2 = Delay + samps1
    newsr = sr
    newsamps = add_2(samps1,samps2)
    new_sound_data = [newsamps, newsr]
